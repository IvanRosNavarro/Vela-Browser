...
const api: PreloadApi = {
  ...
  translation: {
    translate: (input) => call(IPC_CHANNELS.TRANSLATE_TEXT, input),
    getSettings: () => call(IPC_CHANNELS.TRANSLATE_GET_SETTINGS),
    saveSettings: (input) => call(IPC_CHANNELS.TRANSLATE_SAVE_SETTINGS, input),
    detectPageLanguage: (input) => call(IPC_CHANNELS.TRANSLATE_DETECT_PAGE, input),
    showPopup: (input) => call(IPC_CHANNELS.TRANSLATE_SHOW_POPUP, input),
    closePopup: (input) => call(IPC_CHANNELS.TRANSLATE_CLOSE_POPUP, input),
    resizePopup: (input) => ipcRenderer.invoke(IPC_CHANNELS.TRANSLATE_RESIZE_POPUP, input) as Promise<void>,
    copyAndClose: (input) => ipcRenderer.invoke(IPC_CHANNELS.TRANSLATE_COPY_AND_CLOSE, input) as Promise<void>,
    translatePage: (input) => call(IPC_CHANNELS.TRANSLATE_PAGE, input),
    revertPage: (input) => call(IPC_CHANNELS.TRANSLATE_REVERT, input),
    detectLang: (input) => call(IPC_CHANNELS.TRANSLATE_DETECT_LANG, input),
    openConfirmPopup: (input) => call(IPC_CHANNELS.TRANSLATE_CONFIRM_OPEN, input),
    confirmExec: (input) => call(IPC_CHANNELS.TRANSLATE_CONFIRM_EXEC, input),
  },
  resources: {
